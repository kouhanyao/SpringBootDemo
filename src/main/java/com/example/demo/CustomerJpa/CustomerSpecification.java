    ...
    public static <T> Specification<T> byAuto(final EntityManager entityManager, final T example) { //1
        ...
        return new Specification<T>() {
            ...
            public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                List<Predicate> predicates = new ArrayList<>(); //3存储构造的查询条件

                EntityType<T> entity = entityManager.getMetamodel().entity(type);//4获取实体类EntityType

                for (Attribute<T, ?> attr : entity.getDeclaredAttributes()) {//5 对实体类属性循环
                    Object attrValue = getValue(example, attr); //6 获取某一属性的值
                    if (attrValue != null) {
                        if (attr.getJavaType() == String.class) { //7 为字符类型时
                ...
                return predicates.isEmpty() ? cb.conjunction() : cb.and(toArray(predicates, Predicate.class));//11 将条件列表转成Predicate
            }
